...
#import "LoginFindIdController.h"

@interface LoginFindIdController ()

@end

@implementation LoginFindIdController
...
- (IBAction)confirmButtonAction:(id)sender {
    ...
    [param setValue:_idTextField.text forKey:@"name"];
    [param setValue:_phoneNumberTextField.text forKey:@"phone_num"];
    
    [self showIndicator];
    [[MommyRequest sharedInstance] mommyLoginApiService:FindId authKey:nil parameters:param success:^(NSDictionary *data){
        ...
        if([code isEqual:@"0"]){
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                _confirmNumberTimer = [NSTimer scheduledTimerWithTimeInterval:1 target:self selector:@selector(countTimer:) userInfo:nil repeats:YES];
                UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"알림"
                                                                message:[NSString stringWithFormat:@"등록된 아이디는 %@입니다.", [[data objectForKey:@"result"] objectForKey:@"id"]]
                                                               delegate:self
                                                      cancelButtonTitle:@"확인"
                                                      otherButtonTitles:nil, nil];
                alert.tag = 0;
